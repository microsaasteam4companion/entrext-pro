        ...
        <section className="py-32 px-8" id="products">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col items-center text-center mb-24 max-w-5xl mx-auto px-4">
                    <h2 className="font-headline text-5xl md:text-7xl font-black leading-tight reveal active mb-8 whitespace-nowrap">
                    ...
                    {products.map((product, index) => (
                        <div key={index} className="accordion-item group border border-outline-variant/20 rounded-2xl cursor-pointer relative h-[300px] md:h-full overflow-hidden">
                            <div className="absolute inset-0 z-0">
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent opacity-80 transition-opacity"></div>
                            </div>
                            <div className="accordion-content absolute bottom-0 right-0 p-6 md:p-8 z-10 text-right w-full transition-opacity">
                                <div className="flex justify-end items-center gap-3 mb-1">
                                    <div className="text-[10px] md:text-xs font-label uppercase tracking-widest text-white/80">{product.role}</div>
                                    <a href={product.linkedin || "#"} target="_blank" rel="noopener noreferrer" className="z-20 pointer-events-auto" aria-label="LinkedIn Profile">
                                        <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-white hover:text-[#0A66C2] transition-colors cursor-pointer">
                                ...
                                <div className="font-black text-3xl md:text-4xl tracking-tighter text-white group-hover:text-tertiary transition-colors duration-300">{product.brand}</div>
                            </div>
                        </div>
                    ))}
                </div>
